...
it will return to the maze fork that led it to the dead-end, while also creating a virtual barrier (colored lines in the videos below) behind itself, so that it
...
upwards from space D5 to space C5, and there is a barrier ahead (therefore, the robot cannot move to B5) with open spaces on the right (C6)
...
it will go left. Otherwise, it will go right. The second time the robot reaches C5 coming from D5, it will go the opposite direction of how
...
follows the left wall until it reaches the end of the maze. The robot also utilizes a feedback algorithm which centers the robot should
it veer too far off to either side. There is another feedback algorithm, in which, if the robot is stuck at a wall for a while,
it backs up and straightens out. Since the motors in this project did not have encoders, the feedback is from the sensors.

See the attached videos below. They show a maze solver on C++ as well as a wall-following micormouse, each of which uses the algorithms described
in the above two paragraphs, respectively.

https://www.youtube.com/playlist?list=PLBNxZiceX9j3q3Gjt9O5GDz2erVLVYmGN
